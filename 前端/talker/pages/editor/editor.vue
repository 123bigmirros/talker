<template>
	<view class="container">
		<u-field v-model="info.Headline" label="标题" placeholder="请输入标题"><u-button size="mini" slot="right" @click="click()">发布</u-button></u-field>
		<!-- <scroll-view :scroll-x="true">
			<u-tag text="雪月夜" :closeable="true" />
			<u-tag text="雪月夜" :closeable="true" />
		...
		<!-- <u-button type="primary" @click="click()">发布</u-button> -->
	...
	export default {
		data() {
			return {
				...
				readOnly: false,
				formats: {},
				info: getApp().globalData.undoarticle,
		...
		methods: {
			...
			click(){
				(t=>t.editorCtx.getContents({
					success({html}) {
						t.info.Content=html
						t.info.OwnerId=t.id
						uni.sendSocketMessage({
							data:JSON.stringify({Type:2,TypeVariety:0,content:JSON.stringify(t.info)}),
							complete(e){
								console.log(e)
							}
						})
